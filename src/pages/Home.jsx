  ...
  return (
    <>
      ...
      <div className="w-full h-auto flex flex-col justify-center items-center pb-10">
        <span className="w-[72%] flex justify-start items-center p-2 text-2xl mt-4">
          Fresh recommendations
        </span>
        <div className="h-auto w-[75%] flex flex-wrap p-2 gap-2 justify-start items-start">
          {limitedData.map((data, idx) => {
            return (
              <div
                key={idx}
                onClick={() => handleProductDetail(data?.product?.id)}
                className="h-64 w-64 bg-slate-100 p-1 border border-gray-400 rounded relative cursor-pointer"
              >
                <div className="relative h-[60%] w-full border border-gray-400 rounded">
                  ...
                  <div className="absolute top-1 right-1 bg-white h-10 w-10 rounded-3xl flex justify-center items-center cursor-pointer">
                ...
                <div className="w-full h-[40%] pl-3 pt-1 flex flex-col ">
                  ...
                  <span className="text-sm">
                    {data?.product?.year}-{data?.product?.km}
                  </span>
                  <span className="text-sm line-clamp-1">
                    {data?.product?.brand}
                  </span>
                  <span className="text-sm">
                    {data?.product?.city},{data?.product?.state}
                  </span>
                </div>
              </div>
            );
        ...
        <button
          onClick={handleCount}
          className="border-2 hover:border-4 border-black rounded h-12 text-lg p-2 font-bold flex justify-start items-center"
        >
          Load more
        </button>
      </div>
